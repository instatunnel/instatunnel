...
func (c *Client) CreateTunnelWithAuth(localPort int, subdomain string, password string, basicAuth string, customDomain string) (*Tunnel, error) {
...
func (c *Client) StartTunnelWithAuth(localPort int, subdomain string, password string, basicAuth string, customDomain string, showQR bool) error {
	// First create the tunnel
	tunnel, err := c.CreateTunnelWithAuth(localPort, subdomain, password, basicAuth, customDomain)
	if err != nil {
		return fmt.Errorf("failed to create tunnel: %w", err)
	}

	fmt.Printf("🚀 Your app is now live at %s\n", tunnel.URL)
	
	// Show security features
	if password != "" {
		fmt.Printf("🔒 Password protected (visitors need password to access)\n")
	}
	if basicAuth != "" {
		fmt.Printf("🔐 Basic auth enabled (username/password required)\n")
	}
	
	// Try to copy to clipboard
	if utils.TryCopyToClipboard(tunnel.URL) {
		fmt.Printf("📋 URL copied to clipboard!\n")
	}
	
	fmt.Printf("🔗 Forwarding %s -> localhost:%d\n", tunnel.URL, localPort)
	
	if c.apiKey == "" {
		fmt.Printf("💡 Anonymous session - this tunnel will expire in 24 hours\n")
		fmt.Printf("💡 Create a free account to get custom subdomains and longer sessions\n")
	}
	...
	fmt.Printf("n⚡ Ready to receive requests! Press Ctrl+C to stop\n\n")
...
func (c *Client) StartTunnelWithOptions(localPort int, subdomain string, showQR bool) error {
	...
	fmt.Printf("🚀 Your app is now live at %sn", tunnel.URL)
	
	// Try to copy to clipboard
	if utils.TryCopyToClipboard(tunnel.URL) {
		fmt.Printf("📋 URL copied to clipboard!n")
	}
	
	fmt.Printf("🔗 Forwarding %s -> localhost:%dn", tunnel.URL, localPort)
	
	if c.apiKey == "" {
		fmt.Printf("💡 Anonymous session - this tunnel will expire in 24 hoursn")
		fmt.Printf("💡 Create a free account to get custom subdomains and longer sessionsn")
	}
	...
	fmt.Printf("n⚡ Ready to receive requests! Press Ctrl+C to stopnn")
...
func (c *Client) maintainWebSocketConnection(wsURL string, localPort int) error {
	...
	go func() {
		...
		for {
			...
			switch msgType {
			case "connected":
				fmt.Println("✓ Tunnel connected and ready to receive traffic")
			case "request":
				requestCount++
				// Show request counter periodically
				if requestCount%10 == 0 || requestCount <= 5 {
					uptime := time.Since(startTime).Round(time.Second)
					fmt.Printf("📊 Requests: %d | Uptime: %sn", requestCount, uptime)
				}
	...
	for {
		...
		case <-done:
			uptime := time.Since(startTime).Round(time.Second)
			fmt.Printf("n📊 Session Summary: %d requests handled in %sn", requestCount, uptime)
...
func (c *Client) handleTunnelRequestSafe(writeJSON func(interface{}) error, data interface{}, localPort int) {
	...
	fmt.Printf("→ %s %s", tunnelReq.Method, tunnelReq.URL)
...
// ConnectToExistingCustomDomainTunnel connects to an existing tunnel by custom domain
func (c *Client) ConnectToExistingCustomDomainTunnel(domain string, localPort int) error {
	// First check if tunnel exists
	exists, err := c.CheckTunnelExistsByDomain(domain)
	if err != nil {
		return fmt.Errorf("failed to check tunnel existence: %w", err)
	}
	if !exists {
		return fmt.Errorf("tunnel with domain %s not found", domain)
	}

	fmt.Printf("Connecting to existing custom domain tunnel: %s\n", domain)
	fmt.Printf("Forwarding to localhost:%d\n", localPort)

	// Connect via WebSocket using custom domain
	wsURL := c.getWebSocketURLByDomain(domain)
	return c.connectWebSocket(wsURL, localPort)
}

// CheckTunnelExistsByDomain checks if a tunnel exists by custom domain
func (c *Client) CheckTunnelExistsByDomain(domain string) (bool, error) {
	url := fmt.Sprintf("%s/tunnels/domain/%s/exists", c.serverURL, domain)
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return false, err
	}
...
// getWebSocketURLByDomain returns the WebSocket URL for connecting to a tunnel by custom domain
func (c *Client) getWebSocketURLByDomain(domain string) string {
	u, _ := url.Parse(c.serverURL)
	if u.Scheme == "https" {
		u.Scheme = "wss"
	} else {
	...
	u.Path = fmt.Sprintf("/tunnels/domain/%s/connect", domain)
	return u.String()
}
