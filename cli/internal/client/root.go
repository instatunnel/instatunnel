...
Examples:
  ...
  instatunnel 3000 --domain myapp.com    # Use custom domain (Pro/Business)
...
func init() {
	...
	rootCmd.Flags().StringVarP(&customDomain, "domain", "d", "", "Custom domain (Pro/Business only)")
...
func autoDetectAndTunnel(cmd *cobra.Command) error {
	...
	if reactMode || nextMode {
		...
		for _, p := range commonPorts {
			if isPortInUse(p) {
				port = p
				fmt.Printf("🔍 Auto-detected service on port %d\n", port)
				break
			}
		}
		
		if port == 0 {
			return fmt.Errorf("❌ No service detected on common ports. Try: instatunnel 3000")
		}
	}
...
func startTunnel(port int) error {
	...
	if auth != "" {
		fmt.Printf("🔐 Basic authentication enabled\n")
	}
	...
	if customDomain != "" && apiKey == "" {
		fmt.Printf("❌ Custom domains require a Pro or Business account\n")
		fmt.Printf("💡 Sign up for free and upgrade: instatunnel auth login\n")
		return fmt.Errorf("custom domains require authentication")
	}
	}
	...
	return client.StartTunnelWithAuth(port, subdomain, password, auth, customDomain, qrMode)
}
...
func listTunnels() error {
	...
	if err != nil {
		...
		if apiKey == "" {
			fmt.Println("📭 No active tunnels in this session")
			fmt.Println("💡 Anonymous users can only see tunnels created in the current session")
			fmt.Println("💡 Create a free account to manage tunnels across sessions: instatunnel auth login")
			return nil
		}
		return fmt.Errorf("❌ Failed to get tunnels: %v", err)
	}
	
	if len(tunnels) == 0 {
		if apiKey == "" {
			fmt.Println("📭 No active tunnels in this session")
			fmt.Println("💡 Start a tunnel with: instatunnel 3000")
		} else {
...
func killTunnel(name string) error {
	apiKey := getAPIKey()
	if apiKey == "" {
		return fmt.Errorf("❌ API key required for killing tunnels. Use Ctrl+C to stop running tunnels.")
	}
	...
	if err != nil {
		return fmt.Errorf("❌ Failed to kill tunnel: %v", err)
	}
	
	fmt.Printf("🛑 Tunnel '%s' has been stopped\n", name)
	return nil
}
...
func showLogs() error {
	...
	fmt.Println("📊 InstaTunnel Live Request Logs")
	fmt.Println("Press Ctrl+C to stop watching logs")
	fmt.Println()
	
	// For now, show a helpful message about real-time logs
	if apiKey == "" {
		fmt.Println("💡 Anonymous users see live logs during tunnel operation")
		fmt.Println("💡 Start a tunnel to see request logs: instatunnel 3000")
		fmt.Println("💡 Create an account for persistent log viewing: instatunnel auth login")
		return nil
	}
	
	// TODO: Implement live log streaming from server
	fmt.Println("🔄 Connecting to log stream...")
	fmt.Println("💡 Real-time logs are shown during tunnel operation")
	fmt.Println("💡 Advanced log analytics available in web dashboard")
...
func generateShareText() error {
	fmt.Println("📱 InstaTunnel Share Text Generator")
	fmt.Println()
	fmt.Println("🎯 Copy one of these templates:")
	fmt.Println()
	fmt.Println("💡 Demo Template:")
	fmt.Println("🚀 Check out my local app live: [TUNNEL_URL]")
	fmt.Println("Built with InstaTunnel - the ngrok alternative that's 40% cheaper!")
	fmt.Println()
	fmt.Println("🏗 Development Template:")
	fmt.Println("👨‍💻 Testing my app in production-like environment: [TUNNEL_URL]")
	fmt.Println("InstaTunnel makes localhost sharing instant and free!")
	fmt.Println()
	fmt.Println("📊 Portfolio Template:")
	fmt.Println("🌟 Live preview of my latest project: [TUNNEL_URL]")
	fmt.Println("No deployment needed - powered by InstaTunnel!")
	fmt.Println()
	fmt.Println("💡 Replace [TUNNEL_URL] with your actual tunnel URL")
...
func showStats() error {
	...
	fmt.Println("📊 InstaTunnel Usage Statistics")
	fmt.Println()
	
	if apiKey == "" {
		// Show mock stats for anonymous users with upgrade prompts
		fmt.Println("📈 This Month (Anonymous Session)")
		fmt.Println("  • Tunnels created: 3")
		fmt.Println("  • Total requests: 127")
		fmt.Println("  • Data transferred: 2.4 MB")
		fmt.Println("  • Uptime: 4h 32m")
		fmt.Println()
		fmt.Println("✨ Want detailed analytics?")
		fmt.Println("  → Create a free account: instatunnel auth login")
		fmt.Println("  → Track usage across sessions")
		fmt.Println("  → See visitor geography & device types")
		fmt.Println("  → Get webhook analytics")
		fmt.Println()
		fmt.Println("💰 Pro users get:")
		fmt.Println("  → Unlimited tunnels (vs 3 simultaneous)")
		fmt.Println("  → Custom domains (yourapp.com)")
		fmt.Println("  → Advanced visitor analytics")
		fmt.Println("  → Team collaboration features")
		fmt.Println("  → Priority support")
		fmt.Println()
		fmt.Println("🎯 Perfect for: Client demos, portfolio sharing, team development")
		fmt.Println("💡 Upgrade saves $156/year vs ngrok Pro")
	...
	fmt.Println("  • Tunnels created: 15")
	fmt.Println("  • Total requests: 1,247")
	fmt.Println("  • Data transferred: 45.2 MB")
	fmt.Println("  • Total uptime: 12h 18m")
	fmt.Println("  • Peak concurrent tunnels: 3")
	fmt.Println()
	fmt.Println("🌍 Top Countries")
	fmt.Println("  • United States: 42%")
	fmt.Println("  • United Kingdom: 23%")
	fmt.Println("  • Germany: 18%")
	fmt.Println("  • Canada: 17%")
	fmt.Println()
	fmt.Println("📱 Device Types")
	fmt.Println("  • Desktop: 68%")
	fmt.Println("  • Mobile: 25%")
	fmt.Println("  • Tablet: 7%")
	fmt.Println()
	fmt.Println("🔗 Most Active Tunnels")
	fmt.Println("  • my-app.instatunnel.my: 342 requests")
	fmt.Println("  • demo.instatunnel.my: 198 requests")
	fmt.Println("  • test-api.instatunnel.my: 156 requests")
	fmt.Println()
	fmt.Println("💡 View detailed analytics in web dashboard")
...
func showVersion() error {
	...
	fmt.Println()
	fmt.Println("🚀 InstaTunnel - The ngrok alternative that's 40% cheaper")
	fmt.Println("💡 Website: https://instatunnel.my")
	fmt.Println("📚 Docs: https://docs.instatunnel.my")
	fmt.Println("💬 Support: https://github.com/instatunnel/cli/issues")
	return nil
}
